...
#include "../treeminer/timetrack.h"
#include "../treeminer/calcdb.h"
#include "../treeminer/eqclass.h"
#include "../treeminer/stats.h"
#include "../treeminer/hashtable.h"
...
//default flags
bool output = true; //don't print freq subtrees
...
alg_vals alg_type = alltreeminer; //default is to find all freq patterns
...
void parse_args(int argc, char **argv)
{
   ...
   if (argc < 2)
     fout << "usage: sstminer -i<infile> -s <support>\n";
   else{
...
Eqclass* get_F1()
{
  ...
  DBASE_NUM_TRANS = 0;
   ...
   if (MINSUP_PER==0) MINSUP_PER = float(MINSUPPORT)/DBASE_NUM_TRANS;
   fout<<"DBASE_NUM_TRANS: "<< DBASE_NUM_TRANS << endl;
   fout<<"DBASE_MAXITEM: "<< DBASE_MAXITEM << endl;
   fout<<"MINSUPPORT: "<< MINSUPPORT << " (" << MINSUP_PER << ")" << endl;
   fout << "FORMAT:\n"
   << "    prefix: x x x x x x\n"
   << "    #(-1) x - #subtree1\n"
   << "    #(-1) x - #subtree2\n\n";
   ...
   for (i=0,j=0; i < DBASE_MAXITEM; i++) {
         // if (output) fout << i << " - " << itcnt[it_order[i]] << endl;
      if (itcnt[it_order[i]] >= MINSUPPORT) {
         // if (output) fout << it_order[i] 
         //                 << " - " << itcnt[it_order[i]] << endl;
         eq->add_node(j, -1, itcnt[it_order[i]]);
         // if (output) fout << i << " - " << itcnt[it_order[i]] << endl;
  ...
  for (i=0; i < DCB->NumF1; i++){
   ...
   for (i=0; i < DCB->NumF1; i++) {
	...
	//cout << "access " << i << " " << j << endl;
	    ...
	    //if (output) 
	    //  fout << DCB->FreqIdx[i] << " " << DCB->FreqIdx[j] 
            //	   << " - " << itcnt2[i][j] << endl;
         }
      }   
...
void enumerate_freq(Eqclass *eq, int iter)
{
   ...
   for (ni = eq->nlist().begin(); ni != eq->nlist().end(); ++ni){
      ...
      for (nj = eq->nlist().begin(); nj != eq->nlist().end(); ++nj){ 
         // x != y 
         if((*ni)->val != (*nj)->val) continue; // condition 1 in SSTMiner
         // i < j
         ...
         // i > j
         if ((*ni)->pos > (*nj)->pos){
            outs = test_node(iter, neq, (*nj)->val, (*nj)->pos);              //(y,j) -> [Px]
         }
         // i == j
         else{ 
            outs = test_node(iter, neq, (*nj)->val, (*nj)->pos);              //(y,j) -> [Px]
            ins = test_node(iter, neq, (*nj)->val, neq->prefix().size()-1);   // (y,nx) -> [Px]
         }  
...
void main_func()
{
   ...
   for (int i=0; i < stats.size(); i++){
      fout << "F" << i+1 << " - ";
      fout << stats[i].numlarge << " " << stats[i].numcand << endl;
   }
   ...
   fout << "TIME = " << tottime << " sec" << endl;

   //write results to summary file
   ofstream summary("sstminer_summary.out", ios::app); 
   summary << "SSTMINER ";
   switch(sort_type){
...
void sstminer(const string& datafile, const string& outfile, const string& t, double minsup) {
